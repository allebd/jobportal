			...
			<div class="row">
				<div class="col-sm-12 page-content">
					<div class="title-lines">
						<h3 class="mt0">Suspended Jobs</h3>
					</div>

					<div class="clearfix mb30">							
							<a href="<?=base_url();?>jobs_recruit/index" class="btn btn-gray">View Jobs not suspended</a>
					</div>
					<?php if($jobsuspendquery->num_rows() < 1): ?>
						<p>No jobs Suspended</p>
					<?php endif; ?>
					<?php if($jobsuspendquery->num_rows() > 0): ?>
								<?php foreach($jobsuspendquery->result() as $row): ?>
					<div class="candidates-item">
						...
						<ul class="top-btns">
							...
							<li><a href="<?=base_url();?>jobs_recruit/job_unsuspend/<?=$row->job_id?>" class="btn btn-gray fa fa-exclamation" title='Unsuspend Job'></a></li>
							<li><a href="<?=base_url();?>jobs_recruit/job_delete/<?=$row->job_id?>" class="btn btn-gray fa fa-trash-o" title='Delete Job'></a></li>
						</ul>

						<div class="clearfix">
								<a href="<?=base_url();?>jobs_recruit/job_candidates/<?=$row->job_id?>" class="btn btn-default pull-right">View Candidates</a>								
						</div>
						<br>
