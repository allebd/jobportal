				...
				<div class="col-sm-8 page-content mt30">
					<h4>Payment Options</h4>
						...
						<form method=post action=https://simplepay4u.com/process.php>
							...
							<?php foreach($interview->result() as $row): ?>
							<input type=hidden name=price value="<?=$row->price?>">
							<?php endforeach;?>
							<input type=hidden name=quantity value="1">
							<input type=hidden name=ureturn value="<?=base_url();?>applicant/interview_success">
							<!-- <input type=hidden name=unotify value="http://www.mydomain.com/notify.php"> -->
							<input type=hidden name=ucancel value="<?=base_url();?>applicant/interview_failure">
							<input type=hidden name=comments value="Payment Received for Interview Prepping">
					...
					<div class="jobs-item jobs-single-item">
						<?php foreach($interview->result() as $row): ?>
						<p>Pay <strong><?=$row->price?></strong> into the nearest GTBank in the account below. <br> 
						 ...
						 <p>Bank Branch <br>
							...
							Interview Prepping (Amount Deposited) <br>
						 ...
						 <p>Interview tips for your specified topic will be emailed to you immediately your payment reflects.</p>
						 <?php endforeach; ?>
					</div>
				</div>				
			</div>
