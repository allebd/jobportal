			...
			<div class="row">
					...
					<aside>
						<div class="widget sidebar-widget white-container brown-container links-widget">
							<ul>
								...
								<li class="active"><?=anchor('profile_admin/privacyandterms', 'Privacy, Terms & Conditions');?></li>
							</ul>
						</div>
					</aside>
				...
				<div class="col-sm-8 page-content">
					<div class="jobs-item">
						...
						<?php echo form_open_multipart('profile_admin/privacyandterms_update'); ?>
							<h5 class="bottom-line mtm3">Edit Password Reset Message
								<span class="pull-right">
									...
									<a href="<?=base_url();?>profile_admin/privacyandterms" class="btn btn-gray" id='candidate-cert-cancel'>Cancel</a>
								</span>
							</h5>
								<table id='recruiter-desc-general'>
									<tbody>
										<?php foreach($privacyandterms_edit->result() as $row): ?>
										<tr>
											<td>Name</td>
											<td><?=$row->name?></td>
										</tr>
										<tr>
											<?php echo form_hidden('extra_id', $row->extra_id); ?>
											<td>Document Upload</td>
											<?php if($row->upload != ''):?>
											<?php echo form_hidden('upload', $row->upload); ?>
											<td>
												<p><?=$row->upload?></p>
												<p>
