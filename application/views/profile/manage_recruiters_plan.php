			...
			<div class="row">
				...
				<div class="col-sm-8 page-content">
					<div class="jobs-item jobs-single-item">	
						...
						<?php echo form_open_multipart('profile_admin/recruiter_plan_insert'); ?>
							<h5 class="bottom-line mtm3">Edit Industry
								<span class="pull-right">
									<input type='submit' class='btn btn-gray' value='Save' id='candidate-cert-save' />
									<a href="<?=base_url();?>profile_admin/manage_recruiters" class="btn btn-gray" id='candidate-cert-cancel'>Cancel</a>
								</span>
							</h5>
								<table id='recruiter-desc-general'>
									<tbody>
										<?php foreach($recruiters_plan->result() as $row): ?>
										<tr>
											<?php echo form_hidden('id', $row->employer_id); ?>
											<td>Username</td>
											<td><?php echo form_input('name', set_value('name',$row->r_username), 'id="name" class="form-control"');?></td>
										</tr>
										<tr>
											<td>Recruiter Plan</td>
											<td><select name='recruiter_plan' id="recruiter_plan" class="form-control" >
													<option selected='selected' disabled='disabled'>--- Packages ---</option>
													<option value='Hourly Plan'>Hourly Plan</option>
