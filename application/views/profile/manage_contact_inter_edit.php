			...
			<div class="row">
				...
				<div class="col-sm-8 page-content">
					<div class="jobs-item jobs-single-item">
						...
						<?php echo form_open_multipart('profile_admin/inter_edit_update'); ?>
							<h5 class="bottom-line mtm3">Edit Phone Number (International)
								<span class="pull-right">
									<input type='submit' class='btn btn-gray' value='Save' id='candidate-cert-save' />
									<a href="<?=base_url();?>profile_admin/manage_contact" class="btn btn-gray" id='candidate-cert-cancel'>Cancel</a>
								</span>
							</h5>
								<table id='recruiter-desc-general'>
									<tbody>
										<?php foreach($inter_edit->result() as $row): ?>
										<tr>
											<?php echo form_hidden('contact_id', $row->contact_id); ?>
											<td>Phone Number (International)</td>
											<td><?php echo form_input('description', set_value('description',$row->description), 'id="description" class="form-control"');?></td>
										</tr>
										<?php endforeach; ?>																									
									</tbody>
								</table>
