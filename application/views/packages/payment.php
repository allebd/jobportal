				...
				<div class="col-sm-12 page-content">
					<div class="white-container mb0">
						...
						<form method=post action=https://simplepay4u.com/process.php>
							...
							<input type=hidden name=price value="<?=$row->prices_package?>">
							<?php endforeach;?>
							<input type=hidden name=quantity value="1">
							<input type=hidden name=ureturn value="<?=base_url();?>profile_recruit/pay_success">
							<!-- <input type=hidden name=unotify value="http://www.mydomain.com/notify.php"> -->
							<input type=hidden name=ucancel value="<?=base_url();?>profile_recruit/pay_failure">
							<input type=hidden name=comments value="Payment Received for Packages">
							<input type=hidden name=customid value="<?=$this->session->userdata('r_username')?>">
							...
							<input type=image src="https://simplepay4u.com/hlib/images/client_img/simplepaylogo.gif">
						</form>

						<h6>DIRECT BANK DEPOSIT</h6>
						<?php foreach($packages->result() as $row): ?>
						<p>Pay <strong>N<?=$row->prices_package?></strong> into the nearest GTBank in the account below. <br> 
						 ...
						 <p>Your MPS Account will be credited immediately your payment reflects.</p>
						 <?php endforeach; ?>
   					</div>
				</div> 
			</div>
