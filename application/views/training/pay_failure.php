						...
						<?php if($trainings->num_rows() > 0): ?>
								...
								<div class="jobs-item">
									...
									<div class="content">										
										<p><?=$row->details?></p>
										<?php if($row->docone != ''): ?>
										<a href="<?=base_url();?>training/download/<?=$row->docone?>" class="btn btn-default" >Download DOCUMENT / BROCHURE</a>
										<?php endif; ?>
										<a href="<?=base_url();?>training/apply/<?=$row->t_id?>" class="btn btn-default">REGISTER NOW</a>					
									</div>
								</div>		
								<?php endforeach; ?>							
						<?php endif; ?>	
